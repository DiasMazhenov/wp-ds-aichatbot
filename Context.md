...
- Версия `0.5.38`: `LeadRepository::save()` проверяет существование таблицы `wpdsac_leads` через `SHOW TABLES LIKE` и при отсутствии запускает миграцию. Дублирующий retry из `LeadController` убран. Ошибка сохранения при `WP_DEBUG` возвращает `db_message` в REST-ответе и консоли браузера.
