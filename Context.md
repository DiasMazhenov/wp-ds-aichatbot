...
## Последние изменения (сессии 2026-07-21 – 2026-07-22, версии 0.5.55 – 0.5.81)
...
- **Стили главной страницы**: пользователь сообщает о поломке CSS. Все селекторы `.wpdsac-*`, CSS-переменные на `:root` убраны в 0.5.60. **Обнаружено 2026-07-21 через browser automation**: `chat.css` и `chat.js` возвращают 404 на сервере — папка `assets/build/` отсутствует в установленной копии. Файлы присутствуют в Git; после зелёной CI-сборки 0.5.75 требуется чистое обновление через WP Pusher или актуальный artifact ZIP.
