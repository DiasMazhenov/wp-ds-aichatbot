...
- **0.5.92**: DOM перестроен: композер вынесен из `__conversation` и стал прямым потомком `__panel` рядом с `__conversation`. `__panel` всегда прозрачный, `__conversation` — flex-контейнер. Сообщения (`__messages`) занимают всю высоту conversation и поддерживают стеклянный эффект (#backdrop-filter blur + saturate). Новый раздел «Окно сообщений»: режим фона (прозрачный/стекло/сплошной), цвет, прозрачность, размытие, насыщенность, радиус, граница, внутренний блик, внешняя тень, внутренний отступ, отступ до нижней панели. Раздел «Нижняя панель» упрощён до фона, прозрачности, радиуса, padding и gap. Удалены настройки blur/border/shadow композера (теперь это часть окна сообщений).
