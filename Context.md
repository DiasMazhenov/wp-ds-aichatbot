...
- Версия `0.5.44`: `LeadRepository::save()` использует `mysqli_query()` напрямую в обход `$wpdb->query()`, так как на некоторых хостингах `$wpdb` отвергает валидный параметризованный SQL. `LeadRepository::$last_sql` хранит последний запрос для диагностики; `sql_fragment` в ответе доступен только при `WP_DEBUG`. Форма заявок работает.
