...
- **0.5.85**: Архитектура re-engage и QA полностью переработана: отдельный REST `/reengage` с `ReengageService` (cooldown/max count/lead check через transient + HMAC), `ReengageController` не создаёт фейковых пользовательских сообщений. `QuickReplyParser` извлекает `[[WPDSAC_QA|...]]` на сервере, REST возвращает `{reply, quick_replies}`. Контекстные кнопки в `[data-wpdsac-context-actions]` над формой ввода, не внутри сообщений. JS: sessionStorage (`dueAt`, `count`, `lastActivity`), сброс таймера при вводе/клике/форме, `visibilitychange`-aware. Закрытый чат сохраняет ответ в историю + preview в баббл, без дублирования. `ProviderManager::reengage()` — отдельный путь через `wpdsac_reengage_exchange`.
