...
if grep -Eq '^wp-ds-aichatbot/(\.github|scripts|tests|Context\.md|Plan\.md|composer\.(json|lock)|package(-lock)?\.json|phpcs\.xml\.dist|vendor/bin)' <<< "${archive_entries}"; then
