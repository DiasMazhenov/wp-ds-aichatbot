...
Версия `0.5.29` передаёт каждому AI-провайдеру историю текущего browser chat: до 30 сообщений и 20 000 символов. История сохраняется в `sessionStorage` текущей вкладки, поэтому переживает переходы между страницами, санитизируется REST API, считается недоверенными данными и не создаёт отдельного серверного хранилища при выключенном логировании. Системная политика требует продолжать разговор и не здороваться/представляться повторно, если это уже произошло. Исправление ширины кнопки из `0.5.28` сохранено.
