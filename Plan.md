...
Версия `0.5.92`: DOM перестроен — композер вне conversation, панель всегда прозрачная, стеклянные сообщения через backdrop-filter, новые настройки окна сообщений.

### Выполнено в 0.5.92 `.wpdsac-chat__composer` (быстрые кнопки + поле ввода), прозрачная история сообщений, 15 новых настроек нижней панели, горизонтальная прокрутка быстрых кнопок.

### Выполнено в 0.5.92

- [x] DOM: композер вынесен из `__conversation`, стал прямым потомком `__panel`.
- [x] CSS: `__panel` — flex-колонка с прозрачным фоном; `__conversation` — flex:1; `__messages` — стеклянный эффект с blur/saturation.
- [x] Новый раздел «Окно сообщений»: режим (прозрачный/стекло/сплошной), цвет, прозрачность, blur, saturation, radius, border, glare, shadow, padding, отступ до композера.
- [x] Раздел «Нижняя панель»: упрощён до bg/opacity/radius/padding/gap/scroll.
- [x] Удалены настройки композера: blur, border, shadow (теперь часть окна сообщений).
- [x] Preview: `__conversation` + `__composer` внутри `__panel`, мгновенное обновление.
