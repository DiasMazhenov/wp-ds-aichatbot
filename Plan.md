...
Версия `0.5.27` добавляет выбираемый через медиатеку аватар возле каждого сообщения чат-бота, а также возле запроса имени. Приветственный баббл поддерживает задержку (10 секунд по умолчанию), прокрутку 50%, exit intent, мгновенный показ и полное отключение; при ручном открытии запрос имени всегда остаётся внутри чата. Баббл получил полупрозрачный blur, а сетка сообщений и форма ввода исправлены для узких экранов. `{username}` и `(username)` теперь подставляются и в стартовое сообщение на клиенте.
