...
Версия `0.5.28` сохраняет естественную ширину кнопки отправки без обрезания текста, а поле сообщения занимает оставшееся пространство. На экранах до 480 px форма по-прежнему складывается в одну колонку. Возможности аватара, intro triggers и подстановки `{username}` из `0.5.27` сохранены.
