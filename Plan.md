...
Версия `0.5.32` распознаёт имя и телефон в обычном сообщении, не передаёт найденные контакты AI-провайдеру и предзаполняет ими consent form. Исправлено повторное открытие формы после отправки; REST принимает распространённые разделители телефона. При ошибке записи schema v7 автоматически восстанавливает таблицы и повторяет сохранение один раз. Email использует настроенный адрес или fallback на admin email, а сбой `wp_mail()` отражается в debug log и консоли браузера.
