...
Версия `0.5.81`: новые ответы AI выводятся по словам с растущим по содержимому бабблом, настраиваемой скоростью и reduced-motion fallback; незавершённый диалог без заявки отправляется отдельным письмом после настраиваемой паузы.
