...
Версия `0.5.30` детерминированно удаляет повторное приветствие из ответа любого провайдера, если ассистент уже здоровался в истории. Browser memory живёт 24 часа с момента начала разговора; затем история сбрасывается и новое приветствие снова разрешается. Контекст остаётся bounded и untrusted, как в `0.5.29`.
