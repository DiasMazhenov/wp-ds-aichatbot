...
- [x] AI формирует 3–4 динамических варианта ответа из текущего вопроса, истории и знаний сайта; для бинарного вопроса допустимы 2.
- [x] Контекстные варианты остаются в `[data-wpdsac-context-actions]` над формой и не попадают внутрь сообщений.
- [x] PromptGuard распознаёт короткие ответы и уточнения как продолжение подтверждённого on-topic диалога, не ослабляя injection/model-probe проверки.
- [x] Системные инструкции требуют естественного уточнения неоднозначных вопросов, разнообразной формулировки и одного полезного следующего шага.
- [x] SSE-фреймы корректно собираются при любом разделении transport chunks на сервере и в браузере.
- [x] Request lock освобождается до завершения SSE при успехе, rate limit и ошибках провайдера.
- [x] Служебные QA-маркеры и `reasoning_content` DeepSeek не показываются и не сохраняются в истории.
- [x] Ответ посетителя появляется сразу, а очищенный финальный ответ сохраняется после `done`.
...
- [x] **Focus trap**: Tab цикл по focusable элементам chat panel при открытии.
- [x] **Escape**: закрывает lead modal (с возвратом фокуса) и сворачивает чат.
- [x] **ARIA**: `aria-label` на секции чата и панели, `role="complementary"` на панели.
- [x] **Screen-reader**: `aria-live="polite"` + `role="log"` на контейнере сообщений.
