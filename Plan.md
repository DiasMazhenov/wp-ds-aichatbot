...
Версия `0.5.21` добавляет отдельный `AI/PromptGuard`: сервер блокирует явные prompt injection, запросы о модели/провайдере и вопросы вне заданной владельцем тематики до обращения к API. Неизменяемая системная политика также добавляется ко всем AI-провайдерам.
