...
Версия `0.5.26` исправляет перенос текста pre-chat баббла, добавляет выбираемые тихие reply sounds без внешних аудиофайлов и передаёт имя посетителя в AI-контекст. В инструкциях доступен request-scoped тег `{username}`; значение очищается после каждого provider call и считается недоверенными профильными данными.
