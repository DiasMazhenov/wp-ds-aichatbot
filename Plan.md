...
Версия `0.5.31` убирает обязательный pre-chat запрос имени: разговор доступен сразу. При явном contact intent или нажатии «Оставить заявку» ассистент выводит `lead_prompt`, затем открывается форма с обязательными именем, телефоном и согласием; email посетителя больше не запрашивается. Переключатель формы удалён, встроенный contact flow всегда доступен. Быстрые кнопки стоят непосредственно над message input, скрываются после клика и имеют отдельные URL-настройки; пустой URL заявки сохраняет встроенную форму.
