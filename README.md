...
Форма контактов появляется только когда посетитель сам просит связаться, пишет в чат имя с телефоном или нажимает «Оставить заявку». Распознанные данные предзаполняются локально и не передаются AI-провайдеру; посетитель проверяет их и явно подтверждает согласие. Email посетителя не запрашивается. После сохранения на настроенный адрес отправляется bounded transcript текущего чата; если адрес невалиден, используется WordPress admin email.
