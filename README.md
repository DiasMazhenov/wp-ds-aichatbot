...
Опциональную форму имени/email можно включить в `Настройки → DS AI Chatbot → Lead collection`. Форма отображается прямо внутри общего чат-виджета и требует явного согласия с редактируемым текстом. Функция выключена по умолчанию.

Заявка привязывается только к одностороннему hash подписанной chat session; IP и raw session ID не сохраняются. Есть honeypot, отдельный rate limit, retention 1–730 дней, ежедневная очистка и защищённый список в `Инструменты → DS AI Leads`. Экспорт и удаление по email работают через стандартные WordPress privacy tools.
