...
Форма заявки открывается отдельным доступным modal-окном. Быстрые кнопки настраиваются в разделе лидов: можно добавить до восьми собственных действий, открывающих URL или отправляющих подготовленное сообщение. Их цвета, размеры, padding, radius и расстояние регулируются во вкладке внешнего вида.

Для навигационных запросов браузер передаёт только ограниченный список блоков и внутренних ссылок текущего сайта. Ассистент может предложить кнопку перехода, но прокрутка или открытие страницы выполняется только после явного клика посетителя.
