...
Включите `Use website knowledge` в `Настройки → DS AI Chatbot`, затем откройте `Инструменты → DS AI Knowledge` и запустите индексацию. Опубликованные страницы и записи разбиваются на ограниченные фрагменты и сохраняются в отдельной таблице, а не в `wp_options`.

Перед запросом к OpenAI, Claude, Gemini, OpenRouter или WordPress AI Client плагин находит подходящие фрагменты и добавляет их как недоверенный справочный контекст. Изменённые опубликованные записи синхронизируются автоматически. Текущий retrieval использует локальный keyword ranking; semantic embeddings, FAQ, PDF и WooCommerce подключаются следующими адаптерами.
