...
Включите `Использовать знания сайта` в `DS AI Chatbot → Настройки → База знаний`, затем откройте `DS AI Chatbot → База знаний` и запустите индексацию. Опубликованные страницы, записи базы знаний, введённый администратором текст, публичные товары WooCommerce и выбранные PDF разбиваются на ограниченные фрагменты и сохраняются в отдельной таблице, а не в `wp_options`.

Структурированные вопросы и ответы создаются прямо со страницы `DS AI Chatbot → База знаний`: заголовок используется как вопрос/тема, редактор — как проверенный ответ. Там же доступно текстовое поле для инструкций и дополнительных знаний. Управление ограничено администраторами, записи не публикуются как отдельные frontend-страницы.
