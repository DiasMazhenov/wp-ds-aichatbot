...
Во вкладке `DS AI Chatbot → Настройки → Дизайн` параметры разделены на цвета, компоновку/типографику и элементы управления. Отдельно настраиваются шапка, панель, сообщения ассистента и посетителя, поле ввода, кнопка отправки, радиусы, отступы, высота истории, тень, шрифт, позиция и размер круглой свёрнутой кнопки. Live preview переключается между открытым и свёрнутым состояниями. Один дизайн применяется к global, shortcode и Elementor widget.

## База знаний

Включите `Использовать знания сайта` в `DS AI Chatbot → Настройки → База знаний`, затем откройте `DS AI Chatbot → База знаний` и запустите индексацию. Опубликованные страницы, записи, ручные `AI FAQ`, публичные товары WooCommerce и выбранные PDF разбиваются на ограниченные фрагменты и сохраняются в отдельной таблице, а не в `wp_options`.

FAQ создаются через `DS AI Chatbot → AI FAQ`: заголовок используется как вопрос/тема, редактор — как проверенный ответ. Управление FAQ ограничено администраторами, записи не публикуются как отдельные frontend-страницы.
...
Опциональную форму имени/email можно включить в `DS AI Chatbot → Настройки → Лиды`. Форма отображается прямо внутри общего чат-виджета и требует явного согласия с редактируемым текстом. Функция выключена по умолчанию.

Заявка привязывается только к одностороннему hash подписанной chat session; IP и raw session ID не сохраняются. Есть honeypot, отдельный rate limit, retention 1–730 дней, ежедневная очистка и защищённый список в `DS AI Chatbot → Лиды`. Экспорт и удаление по email работают через стандартные WordPress privacy tools.
